<template>
  <el-dialog
    :title="selectData.id ? '编辑文章留言' : '新增文章留言'"
    :visible.sync="show"
...
export default {
  ...
  methods: {
    ...
    async add() {
      ...
      await this.$api.articleComment.createArticle(params)
    ...
    async update() {
      ...
      await this.$api.articleComment.updateArticleComment(this.ruleForm.id, params)
